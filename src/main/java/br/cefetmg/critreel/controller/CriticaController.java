...
public class CriticaController {
    ...
    public ResponseEntity<Critica> delete(@PathVariable Long id,
        ...
        boolean isAdmin = usuarioRepository.isAdmin(requesterId);
        boolean isOwner = critica.getIdUsuario() != null && critica.getIdUsuario().equals(requesterId);

        if (!isAdmin && !isOwner) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Operação permitida apenas para o próprio usuário ou administradores");
        }
